...
The chart will automatically generate a secure Laravel appkey on first deployment.

## Values
Check the [values.yaml](./values.yaml) file for the available settings for this chart and its
dependencies.

### Optional: Custom appkey

By default, a secure appkey is auto-generated on first deployment and preserved across upgrades. To manually set an appkey, use:

```
librenms:
  appkey: base64:YOUR_BASE64_ENCODED_STRING
```

Generate a custom appkey with either:
```bash
# Using openssl (no PHP required)
echo "base64:$(openssl rand -base64 32)"

# Or using Laravel artisan (requires PHP/Laravel)
...
| librenms.appkey | string | `nil` | Laravel appkey used by LibreNMS. If not provided, a secure random appkey will be auto-generated on first deployment and preserved across upgrades. To manually set an appkey, use format: base64:YOUR_BASE64_ENCODED_32_CHAR_STRING Generate with: echo "base64:$(openssl rand -base64 32)" |
...
| librenms.existingSecret | bool | `false` | Existing secret name to use for appkey If specified, the chart will use this secret instead of creating one. The secret must have the key 'appkey' with the Laravel appkey value. Leave empty (false) to auto-generate the secret. |
...
| librenms.initContainer.image.tag | float | `1.37` | tag is the init container image tag to pull. |
...
| librenms.snmp_scanner | object | `{"cron":"15 * * * *","enabled":false,"extraEnvs":{},"resources":{}}` | SNMP network discovery scanner cron job. This job is optional and only use when having snmp network discovery enabled. For this to work either set the 'nets' configuration in the custom config on in the admin interface See the following link for more information: https://docs.librenms.org/Extensions/Auto-Discovery/ |
