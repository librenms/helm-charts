...
**Note:** You can specify the port in the `host` field (e.g., `mysql.example.com:3306`) or separately in the `port` field. If the `host` field contains a port, it takes precedence over the `port` field.
